...
def generate_test_images(dst_dir: str, num_images: int = 10, size : tuple[int, int]=(640, 480)) -> list[dict]:
    ...
    dt_start = dt_curr = datetime(2023, 1, 1, 12, 0, 0)
    ...
    for i in range(num_images):
        ...
        img = Image.fromarray((np.array(img).astype(np.float32) * (1-noise_weight) + noise.astype(np.float32) * noise_weight).clip(0, 255).astype(np.uint8))
